 ...
 *
 * [EnableOAuth2Client][no.nav.security.token.support.client.spring.oauth2.EnableOAuth2Client] i [ApplicationConfig]
 * aktiverer `token-client-spring` sin OAuth2-klientinfrastruktur, som internt bruker [TokenValidationContextHolder]
 * til å hente brukerens innkommende token ved token exchange mot andre tjenester. Denne bønnen sørger for at
...
class TokenValidationContextHolderConfig {
    ...
    class SpringSecurityTokenValidationContextHolder : TokenValidationContextHolder {
        override fun getTokenValidationContext(): TokenValidationContext {
            val validatedTokens =
                (getContext().authentication as? JwtAuthenticationToken)
                    ?.token
                    ?.let { mapOf(it.issuer.toString() to JwtToken(it.tokenValue)) }
                    .orEmpty()
